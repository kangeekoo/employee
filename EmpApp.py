...
def GetEmp():
    ...
    emp_name = "" + first_name + " " + last_name
    return render_template('outputSearch.html', emp_id= emp_id, name=emp_name, email=email, contact=contact, location=location, position=position, pri_skill=pri_skill, hire_date=hire_date, payscale=payscale, emp_image_file=emp_image_file_name_in_s3)
...
def UpdateEmp():
    ...
    try:
        ...
        if emp_image_file.filename != "":
            ...
            try:
                print("Updating employee image file...")
                s3.Bucket(custombucket).put_object(Key=emp_image_file_name_in_s3, Body=emp_image_file)
                bucket_location = boto3.client('s3').get_bucket_location(Bucket=custombucket)
                s3_location = (bucket_location['LocationConstraint'])
    ...
    return render_template('outputUpdate.html', name=emp_name)
